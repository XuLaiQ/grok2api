...
func TestParseImportedCredentialsAcceptsAccountSSOFormat(t *testing.T) {
	adapter := &Adapter{}
	values, err := adapter.ParseImportedCredentials([]byte("user@example.com----password----token-from-account\n" +
		"token-from-account\n" +
		"sso=token-two; x-userid=drop\n"))
	if err != nil {
		t.Fatal(err)
	}
	if len(values) != 2 || values[0].AccessToken != "token-from-account" || values[1].AccessToken != "token-two" {
		t.Fatalf("credentials = %#v", values)
	}
	if values[0].Email != "" || values[0].UserID != "" {
		t.Fatalf("account metadata should not be imported from delimited text: %#v", values[0])
	}
}

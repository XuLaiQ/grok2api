...
func applyEnvironmentOverrides(cfg *Config) error {
	if directory := strings.TrimSpace(os.Getenv("GROK2API_UPDATE_DIR")); directory != "" {
		absolute, err := filepath.Abs(directory)
		if err != nil {
			return fmt.Errorf("解析更新目录: %w", err)
		}
		cfg.Updates.Directory = absolute
	}
	if os.Getenv("GROK2API_UPDATE_MANAGED") == "1" {
		if staticPath := strings.TrimSpace(os.Getenv("GROK2API_FRONTEND_PATH")); staticPath != "" {
...
func resolveRelativePaths(cfg *Config, configPath string) error {
	...
	updatePath := strings.TrimSpace(cfg.Updates.Directory)
	if updatePath != "" && !filepath.IsAbs(updatePath) {
		cfg.Updates.Directory = filepath.Clean(filepath.Join(baseDir, updatePath))
	}
	return nil
}
...
func defaultConfig() Config {
	return Config{
		...
		Updates:  UpdateConfig{Directory: "./data/updates"},

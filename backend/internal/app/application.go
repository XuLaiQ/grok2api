...
func New(ctx context.Context, cfg config.Config, logger *slog.Logger) (*Application, error) {
	...
	updateOptions := updatecheckapp.Options{
		Directory:  cfg.Updates.Directory,
		Repository: os.Getenv("GROK2API_UPDATE_REPOSITORY"),
		Managed:    os.Getenv("GROK2API_UPDATE_MANAGED") == "1",
	}
	if cfg.Deployment.Replicas > 1 {
		updateOptions.DisabledReason = "多实例部署不支持此更新方式，请通过部署平台统一升级"
	}
	updateService := updatecheckapp.NewService(buildinfo.CurrentVersion(), nil, updateOptions)
...
func (a *Application) Run(ctx context.Context) error {
	...
	startBackground("release_check", func(taskCtx context.Context) error {
		if a.updates.Snapshot().Repository != "" {

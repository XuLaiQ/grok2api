...
func TestParseOptionsSupportsSupervisor(t *testing.T) {
	options, err := parseOptions([]string{"--supervise", "--config", "/app/config.yaml", "--listen", "0.0.0.0:8000"})
	if err != nil {
		t.Fatal(err)
	}
	if !options.supervise || options.configPath != "/app/config.yaml" || options.listen != "0.0.0.0:8000" {
		t.Fatalf("options = %#v", options)
	}
}

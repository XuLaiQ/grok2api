...
func TestCheckFindsLatestRelease(t *testing.T) {
	client := &http.Client{Transport: roundTripFunc(func(request *http.Request) (*http.Response, error) {
		if request.URL.String() != "https://api.github.com/repos/chenyme/grok2api/releases/latest" || request.Header.Get("User-Agent") != "grok2api/v3.0.0" {
	...
	service := NewService("v3.0.0", client, Options{Repository: "chenyme/grok2api"})
...
func TestCheckFindsHotfixAfterStableRelease(t *testing.T) {
	...
	service := NewService("v3.0.8", client, Options{Repository: "chenyme/grok2api"})
	snapshot := service.Check(context.Background())

...
export function SettingsPage() {
  ...
  const selectedTab = searchParams.get("tab") ?? "build";
  const activeTab = ["build", "web", "console", "delivery", "policies", "audit", "accounts", "about"].includes(selectedTab) ? selectedTab : "build";
  ...
  return (
    <form className="w-full space-y-5" onSubmit={form.handleSubmit((values) => updateMutation.mutate(values))}>
      <header className="relative sticky top-12 z-30 -mx-2 flex min-h-12 items-center justify-between gap-3 bg-background px-2 py-2 lg:top-20 lg:z-40">
      ...
      {snapshot ? (
        <Tabs value={activeTab} onValueChange={(tab) => setSearchParams((current) => { current.set("tab", tab); return current; }, { replace: true })} className="flex min-w-0 flex-col gap-7 lg:flex-row lg:items-start">
